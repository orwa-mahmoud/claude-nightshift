#!/usr/bin/env bash
# hardhat.sh — Cursor preToolUse guard. Same rules as Claude's hardhat; only the wire format
...
deny() {
  cursor_emit_deny "$(ns_expand_injected_paths "$PROJECT_DIR" "$1")"
  exit 0
}
...
LEASE_COMMAND="$CMD"
case "$TOOL" in Bash | PowerShell | Shell) LEASE_COMMAND="$SCRUBBED" ;; esac
...
if ! ns_hardhat_active; then
  if [ "${NIGHTSHIFT_REVIVAL:-}" = "1" ]; then
    if [ ! -f "$NS/.shift-armed" ] || [ ! -f "$PUNCH" ] || { [ -f "$ENDED" ] && [ ! -L "$ENDED" ]; } \
      || ! ns_lease_nonce_matches "$NS" cursor "$LEASE_NONCE" "$LEASE_GENERATION"; then
...
if ns_hardhat_payload_targets_lease "$TOOL" "$CURSOR_RAW" "$LEASE_COMMAND"; then
...
# Cursor offers no interactive process ancestry this hook can vouch for, so the initial pid and
# start-time lines stay empty. Watchman children carry a unique lease nonce and generation.
ns_shift_unbound cursor hardhat
own_rc=$?
[ "$own_rc" -eq 1 ] && exit 0
...
ns_shift_rebind cursor "" "" hardhat
own_rc=$?
[ "$own_rc" -eq 1 ] && exit 0
[ "$own_rc" -eq 2 ] && deny "$NS_SHIFT_FAIL"
REC="$NS_SHIFT_REC"
...
if ns_hardhat_payload_targets_rules "$TOOL" "$CURSOR_RAW" "$CMD"; then
...
if [ "$TOOL" = "AskUserQuestion" ] \
  || { [ -z "$TOOL" ] && cursor_input_mentions_tool "AskUserQuestion"; }; then
