...
`runtime/release-readiness-evidence.sh` from owner-supplied manifests, CI outputs, package
...
contents, and observed behavior. Use `runtime/release-readiness-evidence.sh public-claims-matrix`
for cross-surface claims; use `runtime/product-truth-evidence.sh doc-claim-matrix` for in-repo
...
- [ ] **Release readiness — compare baseline and candidate; never publish.**
  ...
  - Discovery: record the named baseline ref and candidate ref. Collect or build manifests for
    ...
    `runtime/release-readiness-evidence.sh baseline-compare` before drafting findings.
  - Run `runtime/release-readiness-evidence.sh public-claims-matrix` when public surfaces are in
  ...
  - Produce `runtime/release-readiness-evidence.sh verdict` with status **Ready**, **Not ready**,
    or **Conditionally ready**, listing blockers, non-blockers, and
    `runtime/release-readiness-evidence.sh unmeasured-surfaces` rows. Preserve metric and
  ...
  - Verify: the item gate is green at every commit; baseline-compare and verdict helpers re-run
    clean on the final candidate; public-claims rows show no remaining mechanical drift.
```
