...
- [ ] **Defect hunt — review, fix, re-review until it converges.**
  ...
  - Initialize cycle state with `runtime/defect-cycle.sh init`, then rotate lenses each cycle:
    correctness, state, error handling, concurrency, boundaries, data loss, compatibility, and
    recent-change (`defect-cycle.sh next-lens`). Require reproduction or strong code-path evidence
    for every finding (`defect-cycle.sh record`); track rejected and duplicate findings; rerun the
