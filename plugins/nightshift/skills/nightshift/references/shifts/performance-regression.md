...
- [ ] **Performance regression — compare distributions against a named baseline.**
  - Discovery: record the named baseline ref, measurement source (benchmark, profiler,
    ...
    `runtime/operational-evidence.sh perf-compare` before claiming faster, slower, or unchanged.
    When load or capacity work is in scope, run `runtime/operational-evidence.sh capacity-guard`
  ...
  - Finish with `runtime/operational-evidence.sh measured-summary` listing every scoped surface
