...
- **Resolved policy** — report Doctor's `resolved policy` block: every effective setting with its
 value, source (`built-in`, `rules`, `defaults`, `one-shift`, or `exact-plan`), and expiry
 (`shift`, `permanent`, or `-`). When Doctor warns that a legacy tooling-policy file is still
 present, say so and name Setup as what removes it. When Doctor warns the shift policy is
 malformed, name the field it names. When Doctor warns a deadline/policy mismatch, name both
 values. Do not print inventory command output, credentials, raw evidence, or rule values.
- **Preflight gaps** — report Doctor's preflight fact: which items or Hunt orders need an
 elevation category the resolved policy does not grant, and whether each is allowed for one
 shift, allowed permanently, or still open in `$NS/parking-lot.md`.
...
- **Watchman** — if `$NS/.watch-reason` exists, print line 1 (the stable code) and the
 ...
 detail only.
- **Transition history** — when `$NS/shift-log.md` records stand-down, revival, or host
 changes, summarize those lines in the skill (compact, no secrets). Do not call
 `continuity-handoff.sh transition-history`, `handoff-package`, or `campaign-sequence`.
 Multi-night campaigns are independent bounded shifts; the next night begins only after
 the prior archives or the owner accepts the handoff recorded in the log.
