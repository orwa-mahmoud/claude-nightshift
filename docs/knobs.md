...
| `NIGHTSHIFT_NEVER_COMMIT_PATTERNS` | during a shift, deny a commit whose diff matches this pattern — POSIX `grep -E`; native Windows .NET regular expressions, case-insensitive like POSIX `grep -qiE`. The index is widened to the working tree when the command stages implicitly (`git commit -a`) |
