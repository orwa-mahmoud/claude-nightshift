...
| **night watchman** | `plugin/runtime/claude/watchman.sh` | one per host (`runtime/claude/`, `runtime/codex/`) — revives a session that DIED mid-shift (crash, API outage) by resuming its own conversation; stands down at every honest ending |
