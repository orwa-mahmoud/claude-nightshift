npm-audit: 3 vulnerable packages: 1 critical, 1 high, 1 moderate, 0 low, 0 info

| severity | file | line | code | detail |
| --- | --- | --- | --- | --- |
| critical | minimist | - | <1.2.6 | Prototype Pollution in minimist; fix: available |
| high | semver | - | >=7.0.0 <7.5.2 | semver vulnerable to Regular Expression Denial of Service; fix: available |
| moderate | tough-cookie | - | <4.1.3 | tough-cookie Prototype Pollution vulnerability; fix: none |

showing 3 of 3 items
source: tests/fixtures/normalize/npm-audit/sample.json sha256:7c2318b3ce2c7246e25db0cf37bc871f08297661dc4316f0f782c27a08172447
