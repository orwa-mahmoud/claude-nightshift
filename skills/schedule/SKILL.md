...
```bash
"${CLAUDE_PLUGIN_ROOT}/adapters/schedule.sh" --project "$CLAUDE_PROJECT_DIR" --at <HH:MM>
```
...
generator runs from a terminal with no session — `adapters/schedule.sh` is plain shell and spends
