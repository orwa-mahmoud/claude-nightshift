...
gate held every session; hardhat blocked pushes and vetted committer identity and the staged diff on
every commit; spot-check linted each hook the moment it was written. The live `.nightshift/` state
stayed local — private by default, the same default the plugin sets for you — so this is a curated
...
| 3 | The hooks — clock-out gate, hardhat, spot-check + bats suite | done — 7 commits |
...
2026-07-13 22:13 · feat(hooks): spot-check
...
- **clock-out gate** — no session ended while a box was open; the contract was re-injected on every
  premature stop.
- **hardhat** — every `git push` denied; every commit checked for the expected identity and swept for
  private identifiers before it was allowed.
- **spot-check** — each hook `bash -n`'d and shellcheck'd as it landed; the guard even refused a Bash
  command that merely contained a push, so the build had to route around its own rules.
